...
export default function LoginPage() {
    ...
    return (
        <div className="min-h-screen bg-slate-900 relative flex items-center justify-center px-4 py-10 sm:px-6 lg:px-8">
            ...
            <div className="absolute bottom-0 -right-10 w-72 h-72 bg-purple-600 rounded-full mix-blend-multiply filter blur-3xl opacity-10 animate-blob animation-delay-2000"></div>

            {/* Card */}
            <div className="relative w-full max-w-md bg-white rounded-3xl shadow-2xl border border-slate-100 p-6 sm:p-8">

                {/* Header */}
                <div className="flex flex-col sm:flex-row items-center gap-5 sm:gap-4 mb-8 sm:mb-10 text-center sm:text-left">
                    <div className="w-20 h-20 relative bg-slate-50 rounded-2xl p-4 shadow-sm border border-slate-100">
                        <Image
                            src="/logo-blue.png"
                            alt="TAFES Logo"
                            fill
                ...
                <form onSubmit={handleLogin} className="space-y-5">
                    ...
                    {/* Username */}
                    <div>
                        <label className="block text-slate-700 text-sm font-semibold mb-2 ml-1 italic opacity-80">
                            Username
                        </label>
                        <div className="relative">
                            <User className="absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-400" size={18} />
                            <input
                                ...
                                value={username}
                                onChange={(e) => setUsername(e.target.value)}
                                className="w-full pl-11 pr-4 py-3.5 bg-slate-50 border border-slate-200 rounded-2xl focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 text-slate-900 transition-all font-medium placeholder-slate-400 text-sm sm:text-base"
                                placeholder="Enter admin username"
                                required
                            />
                        </div>
                    ...
                    <button
                        type="submit"
                        disabled={loading}
                        className="w-full bg-blue-600 hover:bg-blue-700 disabled:bg-blue-400 text-white font-bold py-3.5 sm:py-4 rounded-2xl shadow-lg shadow-blue-500/30 transition-all active:scale-[0.98] flex items-center justify-center gap-2 mt-4 text-sm sm:text-base"
                    >
                        {loading ? (
                            <>
                                <Loader2 className="animate-spin" size={20} />
